import {
  ...
  export class VideosService {
     ...
     * Get a video by id.
     */
     async getSharedVideoLinkById(id: string): Promise<{ expiry: Date, video: { id: string, url: string } }> {
        return this.sharedLinkRepository.findOne({
            where: {
                id
            },
            relations: ['video'],
     ...
     */
    async trimVideo(videoId: string, start: number, end: number): Promise<Video> {
      ...
      await this.processVideoTrim(video.url, start?.toString() || '0', end?.toString() || video.durationInSeconds.toString(), trimmedFilePath);
